...
class _LiptonAdminDashboardState extends State<LiptonAdminDashboard>
  ...
  Future<void> fetchOrders() async {
    try {
      final response =
      await http.get(Uri.parse("http://172.20.2.13:5000/api/orders/lipton"));
      ...
      flexibleSpace: FlexibleSpaceBar(
        background: Stack(
          ...
          children: [
            CachedNetworkImage(
              imageUrl:
              "https://images.unsplash.com/photo-1544787210-2213d84ad96b?q=80&w=1963",
              fit: BoxFit.cover,
            ),
            Container(
              decoration: BoxDecoration(
  ...
  Widget _buildStatsGrid(int total, double revenue) {
    return GridView.count(
      ...
      children: [
        _buildStatCard(
            "Daily Revenue", "₹${revenue.toStringAsFixed(0)}",
    ...
    return SliverPadding(
      ...
      sliver: SliverList(
        delegate: SliverChildBuilderDelegate((context, index) {
          ...
          return _buildOrderCard(order, itemsSummary);
        }, childCount: orders.length),
      ),
    );
  }

  Widget _buildOrderCard(Map order, String itemsSummary) {
    return Container(
